...
"""
   ...
   if InitRMSD_unsorted < 0.001:
      ...
      write_to_xyz(NA_a, name, b_labels, b_coords)
      print("Best alignment written to " + str(name))
      sys.exit()
   ...
   for i in range(len(B_t)):
      ...
      if num_uniq == 1:
         ...
         while l < num_uniq:
            ...
            cost_matrix = np.empty((len(vars()[b_Coords[Uniq[l]]]), len(vars()[a_Coords[Uniq[l]]])))
            for i, b in enumerate(np.array(vars()[b_Coords[Uniq[l]]])):
                for j, a in enumerate(np.array(vars()[a_Coords[Uniq[l]]])):
